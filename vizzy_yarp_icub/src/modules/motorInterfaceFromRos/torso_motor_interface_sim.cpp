...
//    Thread1():Thread(){}
    ...
    double my_timeout=0.5;
	while (motionDone==false && my_timeout>0)
...
//    ok &= dd.view(icm);
    ...
    while (!connected){
	connected = subscriber.topic("/vizzy/torso_joint/cmd");
        if (!connected){
	    std::cout << "Failed to subscriber to /vizzy/torso_joint/cmd" << std::endl;
            Time::delay(1);
        }
	else
