...
int main(int argc, char *argv[]) {
  ...
  bool receiver1Mux1Ok = receiverBuff1Mux1.open("/ballPointBridge/mux1/receiver1");

  Port outputPort;
  outputPort.promiseType(Type::byNameOnWire("geometry_msgs/PointStamped"));
  outputPort.setWriteOnly();
  bool outputOk = outputPort.open("/ball_point@/yarp/ballPointBridge");
  ...
  while (!allConnected){
    ...
    if (!portsConnected[0][1]){
      portsConnected[0][1] = yarp.connect("/pf3dTracker/data:o", receiverBuff1Mux1.getName());
      if (!portsConnected[0][1])
        connectedNumber--;
  ...
  while(true){
    ...
    list_1.add("l_camera_vision_link");
